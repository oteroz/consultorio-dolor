...
export default function Pacientes() {
  ...
  useEffect(() => {
    setLoading(true);
    const t = setTimeout(() => {
      api.get('/patients' + (q ? '?q=' + encodeURIComponent(q) : ''))
        ...
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-slate-700 font-medium">
              {q ? `Sin resultados para "${q}"` : 'Aun no hay pacientes'}
            </p>
            {!q && (
              <p className="text-slate-500 text-sm mt-1">Crea el primero con el boton de arriba.</p>
            )}
          </div>
        ) : (
